FIRST_WIRE = "R997,D99,R514,D639,L438,D381,L251,U78,L442,D860,R271,U440,L428,U482,R526,U495,R361,D103,R610,D64,L978,U587,L426,D614,R497,D116,R252,U235,R275,D882,L480,D859,L598,D751,R588,D281,R118,U173,L619,D747,R994,U720,L182,U952,L49,D969,R34,D190,L974,U153,L821,U593,L571,U111,L134,U111,R128,D924,R189,U811,R100,D482,L708,D717,L844,U695,R277,D81,L107,U831,L77,U609,L629,D953,R491,D17,R160,U468,R519,D41,R625,D501,R106,D500,R473,D244,R471,U252,R440,U326,R710,D645,L190,D670,L624,D37,L46,D242,L513,D179,R192,D100,R637,U622,R322,U548,L192,D85,L319,D717,L254,D742,L756,D624,L291,D663,R994,U875,R237,U304,R40,D399,R407,D124,R157,D415,L405,U560,R607,U391,R409,U233,R305,U346,L233,U661,R213,D56,L558,U386,R830,D23,L75,D947,L511,D41,R927,U856,L229,D20,L717,D830,R584,U485,R536,U531,R946,U942,R207,D237,L762,U333,L979,U29,R635,D386,R267,D260,R484,U887,R568,D451,R149,U92,L379,D170,R135,U799,L617,D380,L872,U868,R48,U279,R817,U572,L728,D792,R833,U788,L940,D306,R230,D570,L137,U419,L429,D525,L730,U333,L76,D435,R885,U811,L937,D320,R152,U906,L461,U227,L118,U951,R912,D765,L638,U856,L193,D615,L347,U303,R317,U23,L139,U6,L525,U308,L624,U998,R753,D901,R556,U428,L224,U953,R804,D632,L764,U808,L487,U110,L593,D747,L659,D966,R988,U217,L657,U615,L425,D626,L194,D802,L440,U209,L28,U110,L564,D47,R698,D938,R13,U39,R703,D866,L422,D855,R535,D964,L813,D405,R116,U762,R974,U568,R934,U574,R462,D968,R331,U298,R994,U895,L204,D329,R982,D83,L301,D197,L36,U329,R144,U497,R300,D551,L74,U737,R591,U374,R815,U771,L681"
SECOND_WIRE = "L997,D154,R652,U379,L739,U698,R596,D862,L125,D181,R786,U114,R536,U936,L144,U936,R52,U899,R88,D263,R122,D987,L488,U303,R142,D556,L691,D769,L717,D445,R802,U294,L468,D13,R301,D651,L242,D767,R465,D360,L144,D236,R59,U815,R598,U375,R645,U905,L714,U440,R932,D160,L420,U361,L433,D485,L276,U458,R760,D895,R999,U263,R530,U691,L918,D790,L150,U574,R800,U163,R478,U112,L353,U30,L763,U239,L353,U619,R669,D822,R688,U484,L678,D88,R946,D371,L209,D175,R771,D85,R430,U16,R610,D326,R836,U638,L387,D996,L758,U237,L476,U572,L456,U579,L457,D277,L825,U204,R277,U267,L477,D573,L659,D163,L516,D783,R762,U146,L387,U700,R911,U335,L115,D887,R677,U312,R707,U463,L743,U358,L715,D603,R966,U21,L857,D680,R182,D977,L279,U196,R355,D624,L434,U410,R385,U47,L999,D542,L453,D735,R781,U115,R814,U110,R344,D139,R899,D650,L118,D774,L227,D140,L198,D478,R115,D863,R776,D935,R473,U722,R555,U528,L912,U268,R776,D223,L302,D878,R90,U52,L595,U898,L210,U886,R161,D794,L846,U404,R323,U616,R559,U510,R116,D740,L554,U231,R54,D328,L56,U750,R347,U376,L148,U454,L577,U61,L772,D862,R293,U82,L676,D508,L53,D860,L974,U733,R266,D323,L75,U218,L390,U757,L32,D455,R34,D363,L336,D67,R222,D977,L809,D909,L501,U483,L541,U923,R97,D437,L296,D941,L652,D144,L183,U369,L629,U535,L825,D26,R916,U131,R753,U383,L653,U631,R280,U500,L516,U959,R858,D830,R357,D87,R885,D389,L838,U550,R262,D529,R34,U20,L25,D553,L884,U806,L800,D988,R499,D360,R435,U381,R920,D691,R373,U714,L797,D677,L490,D976,L734,D585,L384,D352,R54,D23,R339,D439,L939,U104,L651,D927,L152"

FIRST_WIRE = FIRST_WIRE.split(',')
SECOND_WIRE = SECOND_WIRE.split(',')
######### PART 2 ############

# all intersections were found in PART 1
# first number - X, second number - Y

intersections = [
2565,288     ,
2827,288     ,
3208,288     ,
2493,485     ,
2565,609     ,
2161,738     ,
2717,849     ,
2717,949     ,
2717,976     ,
2178,1037    ,
2454,1037    ,
2537,1037    ,
2565,1037    ,
1408,1077    ,
916,1083     ,
1512,1083    ,
1512,1134    ,
1790,1134    ,
2887,1159    ,
2912,1159    ,
2709,1250    ,
2887,1250    ,
2912,1250    ,
2938,1250    ,
3307,1250    ,
3399,1250    ,
3549,1250    ,
2919,1310    ,
2375,1442    ,
2537,1442    ,
2537,1471    ,
2919,1486    ,
2379,1590    ,
2645,1590    ,
2726,1590    ,
2919,1590    ,
2645,1632    ,
2726,1632    ,
2919,1632    ,
2709,1774    ,
3557,1871    ,
2709,2000    ,
2645,2006    ,
3089,2015    ,
3089,2099    ,
3511,2415    ,
2953,2428    ,
3089,2428    ,
2953,2484    ,
3089,2484    ,
2742,2495    ,
2749,2495    ,
3567,2587    ,
2953,2651    ,
3567,2651    ,
2444,2678    ,
2953,2794    ,
2749,2957    ,
2500,2979    ,
2500,3041    ,
2500,3461    ,
4284,3467    ,
4359,3467    ,
4373,3467    ,
2500,3492    ,
4439,3529    ,
3795,3577    ,
3930,3577    ,
3999,3577    ,
4009,3577    ,
4439,3596    ,
4440,3606    ,
4496,3606    ,
4777,3606    ,
4236,3653    ,
4440,3653    ,
4777,3653    ,
2651,3692    ,
2674,3692    ,
2860,3692    ,
3124,3692    ,
3200,3692    ,
8975,3821    ,
9604,3821    ,
5338,4076    ,
5517,4076    ,
9746,4185    ,
5665,4346    ,
4236,4433    ,
4461,4456    ,
5820,4457    ,
412,4634     ,
5758,4705    ,
5874,4705    ,
304,4738     ,
774,4738     ,
876,4738     ,
963,4738     ,
5874,4797    ,
2559,4893    ,
2561,4893    ,
5220,4949    ,
5318,4949    ,
5252,4967    ,
5400,4967    ,
412,4991     ,
988,4991     ,
412,5020     ,
988,5020     ,
9537,5042    ,
2559,5051    ,
2561,5051    ,
2850,5068    ,
2970,5068    ,
188,5127     ,
304,5127     ,
355,5127     ,
774,5127     ,
876,5127     ,
963,5127     ,
2559,5159    ,
2561,5159    ,
2758,5159    ,
2850,5173    ,
5400,5197    ,
4795,5233    ,
5199,5233    ,
2349,5234    ,
2349,5301    ,
2949,5332    ,
2970,5391    ,
2447,5393    ,
2758,5393    ,
355,5639     ,
4876,5654    ,
4910,5654    ,
3732,5665    ,
2447,5681    ,
2780,5801    ,
3109,5801    ,
3343,5801    ,
2447,5906    ,
2447,6144    ,
2447,6208    ,
2110,6278    ,
2237,6278    ,
2426,6278    ,
1638,6487    ,
1638,6666    ,
1638,7140    ,
1227,7187    ,
2617,9656    ,
2827,9656    ,
3297,9970
]

# intersections = [
#     246,54,
#     255,89,
#     255,96,
#     258,112,
# ]

init_y = 2000
init_x = 2000

steps_first = []

for i in range(len(intersections)-1):
    steps_taken = 0
    curr_x = init_x
    curr_y = init_y
    for path in FIRST_WIRE:
        if path[0] == "R":
            right = int(path[1:])
            while right > 0:
                steps_taken += 1
                curr_x += 1
                if curr_x == intersections[i] and curr_y == intersections[i+1]:
                    print(f'Intersection: {intersections[i]},{intersections[i+1]}, Steps taken: {steps_taken}')
                    steps_first.append(steps_taken)
                right -= 1
        if path[0] == "L":
            left = int(path[1:])
            while left > 0:
                steps_taken += 1
                curr_x -= 1
                if curr_x == intersections[i] and curr_y == intersections[i+1]:
                    print(f'Intersection: {intersections[i]},{intersections[i+1]}, Steps taken: {steps_taken}')
                    steps_first.append(steps_taken)
                left -= 1
        if path[0] == "U":
            up = int(path[1:])
            while up > 0:
                steps_taken += 1
                curr_y -= 1
                if curr_x == intersections[i] and curr_y == intersections[i+1]:
                    print(f'Intersection: {intersections[i]},{intersections[i+1]}, Steps taken: {steps_taken}')
                    steps_first.append(steps_taken)
                up -= 1
        if path[0] == "D":
            down = int(path[1:])
            while down > 0:
                steps_taken += 1
                curr_y += 1
                if curr_x == intersections[i] and curr_y == intersections[i+1]:
                    print(f'Intersection: {intersections[i]},{intersections[i+1]}, Steps taken: {steps_taken}')
                    steps_first.append(steps_taken)
                down -= 1

print("SECOND WIRE")

steps_second = []

for i in range(len(intersections)-1):
    steps_taken = 0
    curr_x = init_x
    curr_y = init_y
    for path in SECOND_WIRE:
        if path[0] == "R":
            right = int(path[1:])
            while right > 0:
                steps_taken += 1
                curr_x += 1
                if curr_x == intersections[i] and curr_y == intersections[i+1]:
                    #print(f'Intersection: {intersections[i]},{intersections[i+1]}, Steps taken: {steps_taken}')
                    steps_second.append(steps_taken)
                right -= 1
        if path[0] == "L":
            left = int(path[1:])
            while left > 0:
                steps_taken += 1
                curr_x -= 1
                if curr_x == intersections[i] and curr_y == intersections[i+1]:
                    #print(f'Intersection: {intersections[i]},{intersections[i+1]}, Steps taken: {steps_taken}')
                    steps_second.append(steps_taken)
                left -= 1
        if path[0] == "U":
            up = int(path[1:])
            while up > 0:
                steps_taken += 1
                curr_y -= 1
                if curr_x == intersections[i] and curr_y == intersections[i+1]:
                    #print(f'Intersection: {intersections[i]},{intersections[i+1]}, Steps taken: {steps_taken}')
                    steps_second.append(steps_taken)
                up -= 1
        if path[0] == "D":
            down = int(path[1:])
            while down > 0:
                steps_taken += 1
                curr_y += 1
                if curr_x == intersections[i] and curr_y == intersections[i+1]:
                    #print(f'Intersection: {intersections[i]},{intersections[i+1]}, Steps taken: {steps_taken}')
                    steps_second.append(steps_taken)
                down -= 1

fewest_steps = 10000000

# gives error, but output correct answer before the error
for i in range(len(steps_first)-1):
    if steps_first[i] + steps_second[i] < fewest_steps:
        fewest_steps = steps_first[i] + steps_second[i]
        print(f'Fewest steps: {fewest_steps}')